...
class DetailViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        viewWhite.backgroundColor = UIColor.grayColor()
        view.addSubview(viewWhite)
        imageView = UIImageView(image: img)
        imageView.frame = CGRect(x: 0, y: 65, width: screenSize.width, height: screenSize.width)
        imageView.contentMode = UIViewContentMode.ScaleAspectFit
        view.addSubview(imageView)
        let textFrame = CGRect(x: 5, y: 65 + screenSize.width + 10, width: screenSize.width/1.6, height: 20)
        textLabel = UILabel(frame: textFrame)
        textLabel.textColor = UIColor.blueColor()
        textLabel.font = UIFont.systemFontOfSize(UIFont.smallSystemFontSize())
        textLabel.textAlignment = .Left
        textLabel.text = "Username: " + self.user
        view.addSubview(textLabel)
        let textFrame2 = CGRect(x: 5, y: 65 + screenSize.width + 10, width: screenSize.width/1.6, height: 20)
        textLabelLikes = UILabel(frame: textFrame2)
        textLabelLikes.textColor = UIColor.redColor()
        ...
        button.frame = CGRect(x: screenSize.width - screenSize.width/5, y: screenSize.width + 90, width: 60, height: 20)
        button.backgroundColor = UIColor.whiteColor()
        button.setTitle("❤️ Like", forState: UIControlState.Normal)
        button.addTarget(self, action: "buttonAction:", forControlEvents: UIControlEvents.TouchUpInside)
        self.view.addSubview(button)
    }
    
    func buttonAction(sender:UIButton!) {
        if (self.button.backgroundColor == UIColor.whiteColor()) {
